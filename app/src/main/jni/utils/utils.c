...
int com_open(char *dev) {
	...
	int r = 0;
	if(DevicesFlag == 0){
		r = VerifyEztorProduct();
		if(r == 1){
		  DevicesFlag = 1;
		  __android_log_print(ANDROID_LOG_INFO, "utils","Verify Eztor key success!");
		}else{
		  __android_log_print(ANDROID_LOG_INFO, "utils","Verify Eztor key false, return = %d!",r);
		  return r;
		}
	}
...
int com_select(int fd, int usec)
{
	fd_set rfds;
	int len=0,r=0;
	struct timeval tv;
 ...
 *  判断是否为Eztor产品
 *  验证成功返回1  失败
 *  返回 -1001 （打开/dev/disp 失败 ）
 *  返回 -1002  验证Eztor key失败
 */
int gpio_get_lcd_bright[4]					={0,0,0,0};    //get lcd bright
int DISP_TEST=0x288;
int nEztor_key = 888;
int VerifyEztorProduct()
{
	int dispfh = 0;
	int r = -1;
